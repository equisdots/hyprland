...
while true; do
    # Fetch local version
    LOCAL_VERSION=$(source ~/.local/state/equisdots-version 2>/dev/null && echo "$LOCAL_VERSION")
    LOCAL_VERSION=${LOCAL_VERSION:-"Unknown"}
    
    # Fetch remote version
    REMOTE_VERSION=$(curl -m 5 -s https://raw.githubusercontent.com/equisdots/hyprland/main/install.sh | grep '^INSTALL_VERSION=' | cut -d'"' -f2)
    ...
    if [[ -n "$REMOTE_VERSION" && "$LOCAL_VERSION" != "Unknown" && "$LOCAL_VERSION" != "$REMOTE_VERSION" ]]; then
        ...
        if [[ "$NEWEST" == "$REMOTE_VERSION" ]]; then
            ...
            if [[ ! -f "$CACHE_FILE" ]] || [[ "$(cat "$CACHE_FILE")" != "$REMOTE_VERSION" ]]; then
                ...
                notify-send -t 15000 -a 'equisdots' -u normal 'Update Available' "A new version ($REMOTE_VERSION) is ready! Click the update icon in the topbar to install."
